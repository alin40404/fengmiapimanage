...
function Dialog(content, options)
{
    var defaults = { // 默认值。 
        ...
        closeText:'&times;', // 关闭按钮文字，若不想显示关闭按钮请通过CSS设置其display为none 
        draggable:true,     // 是否移动 
        modal:true,         // 是否是模态对话框 
        center:true,        // 是否居中。 
        fixed:true,         // 是否跟随页面滚动。
    ...
    options.id = options.id ? options.id : 'to_dialog-' + Dialog.__count; // 唯一ID
    ...
    var barHtml = !options.showTitle ? '' :
        '<div class="bar"><span class="title">' + options.title + '</span><span class="closeTips"></span><a class="close">' + options.closeText + '</a></div>';
	var icon=!options.icon ? '' :'<img src="'+options.icon+'" alt="" title="" />';
	var footerHtml='',ok='',cancel='';
	if(options.ok){ok='<a class="ok btn btn-small btn-primary">'+options.ok+'</a>';}
	if(options.cancel){cancel='<a class="cancel btn btn-small ">'+options.cancel+'</a>';}
	if(options.ok||options.cancel){footerHtml='<div class="to_dialog_footer">'+ok+cancel+'</div>';}
    var dialog = $('<div id="' + options.id + '" class="to_dialog">'+barHtml+'<div class="to_dialog_content">'+icon+'</div>'+footerHtml+'</div>').hide();
    $('body').append(dialog);
	that=this;
    ...
    var init = function()
    {
        ...
        if(options.modal)
        {
            $('body').append('<div id="' + overlayId + '" class="to_dialog-overlay"></div>');
		...
		dialog.find('.cancel').bind('click', this.close);
		dialog.find('.ok').bind('click', this.ok);
    ...
    {
        var div = dialog.find('.to_dialog_content');
        if('object' == typeof(c))
        {
    ...
    this.show = function()
    {
        ...
        // 自动关闭 
        if(0 != options.time){_time=options.time;_time=parseInt(_time);_time=isNaN(_time)?3000:_time;_time=_time/1000;dialog.find('.bar>.closeTips').html('('+_time+'秒后关闭！)');intervalTime=setInterval(function(){_time--;dialog.find('.bar>.closeTips').html('('+_time+'秒后关闭！)');},1000);timeId = setTimeout(this.close, options.time);}
	...
	this.ok=function(){
		if(undefined != options.beforeOk && !options.beforeOk())
        {   return;  }
		if(undefined != options.afterOk){   options.afterOk(that); }
	}
